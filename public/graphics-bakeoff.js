...
const requestedMode = (params.get('mode') || 'C').toUpperCase()
const MODE = ['A', 'B', 'C'].includes(requestedMode) ? requestedMode : 'C'
...
function radialGlow(x, y, radius, color, alpha) {
  ...
  for (let i = 12; i >= 1; i -= 1) {
    const p = i / 12
    c.addChild(new Graphics().circle(x, y, radius * p).fill({ color, alpha: alpha * (1 - p) * 0.24 }))
  }
  return c
}

// ---------------------------------------------------------------------------
// SAME ROOM / SAME ACTORS FOR ALL MODES
// ---------------------------------------------------------------------------
...
if (MODE !== 'A') {
  world.addChild(radialGlow(220, 235, 270, 0x7137c8, 0.7))
  world.addChild(radialGlow(1080, 250, 250, 0xff244f, 0.48))
  world.addChild(radialGlow(650, 520, 330, 0x512497, 0.34))
  ...
  for (let i = 0; i < 185; i += 1) {
    ...
    const r = 1 + rand() * 10
    grime.circle(x, y, r).fill({ color: rand() > 0.82 ? 0x51142b : 0x2b1e31, alpha: 0.08 + rand() * 0.26 })
  }
  world.addChild(grime)
}

const sign = panel(490, 125, 300, 72, MODE === 'A' ? 0x28252b : 0x09060e, 0.98, MODE === 'A' ? 0x706a75 : 0x8951df)
world.addChild(sign)
const signText = new Text({
  text: MODE === 'A' ? 'RAW PIXI BASELINE' : 'BEAN COUNTY MALL\nAFTER HOURS',
  style: {
    fontFamily: 'Arial Black, Impact, sans-serif',
    fontSize: MODE === 'A' ? 22 : 24,
    fontWeight: '900',
    fill: MODE === 'A' ? 0xd7d2dd : 0xd3baff,
    align: 'center',
    letterSpacing: 2,
  },
})
...
if (MODE !== 'A') {
  ...
  for (const [copy, x, y, w, h, color] of slogans) {
...
// Persistent world blood is presentation-only.
if (MODE !== 'A') {
  for (let i = 0; i < 46; i += 1) {
    ...
    blood.alpha = 0.18 + rand() * 0.48
    decals.addChild(blood)
  }
}

// 120 identical zombies in all three modes. Only presentation changes.
...
for (let i = 0; i < zombieCount; i += 1) {
  ...
  if (MODE !== 'A') {
    const shadow = new Graphics().ellipse(0, 25, 46, 15).fill({ color: 0x000000, alpha: 0.38 })
    holder.addChild(shadow)
  }

  const sprite = new Sprite(zombieTexture)
  sprite.anchor.set(0.5)
  const size = 0.63 + rand() * 0.23
  sprite.scale.set(size)
  if (MODE === 'A') sprite.tint = 0xd7d3da
  else sprite.tint = i % 11 === 0 ? 0xe5c8ff : 0xcbb9dc
  holder.addChild(sprite)
  holder.position.set(x, y)
  holder.zIndex = Math.round(y)
  actors.addChild(holder)
  zombies.push({ holder, sprite, baseX: x, baseY: y, phase: rand() * 10 })
}

// Player: authored art in every mode so the test is renderer/effects, not character identity.
...
// ---------------------------------------------------------------------------
// B/C: POOLED GPU-FRIENDLY PRESENTATION PARTICLES
// ---------------------------------------------------------------------------
const particlePool = []
let particleCursor = 0
const particleCapacity = MODE === 'A' ? 0 : 1800
let particleTexture = null
if (MODE !== 'A') {
  const dot = new Graphics().circle(6, 6, 5).fill({ color: 0xffffff })
  particleTexture = app.renderer.generateTexture(dot)
...
function spawnParticleBurst(x, y, count = 900, palette = [0xb41135, 0xff315c, 0xffb3c4, 0x9f61ff]) {
  if (MODE === 'A') return
  const n = Math.min(count, particleCapacity)
  for (let i = 0; i < n; i += 1) {
    const p = particlePool[particleCursor]
    particleCursor = (particleCursor + 1) % particleCapacity
    const angle = rand() * Math.PI * 2
    const speed = 80 + rand() * 620
    p.sprite.visible = true
    p.sprite.position.set(x + (rand() - 0.5) * 36, y + (rand() - 0.5) * 24)
    p.sprite.scale.set(0.35 + rand() * 1.6)
    p.sprite.tint = palette[Math.floor(rand() * palette.length)]
    p.sprite.alpha = 0.95
    p.vx = Math.cos(angle) * speed
    p.vy = Math.sin(angle) * speed
    p.maxLife = p.life = 0.3 + rand() * 1.0
    p.spin = (rand() - 0.5) * 14
  }
}

// ---------------------------------------------------------------------------
// C: RAPIER OWNS CHAOS OBJECTS ONLY — NEVER HIT/DAMAGE AUTHORITY
// ---------------------------------------------------------------------------
let RAPIER = null
...
if (MODE === 'C') {
  try {
    const rapierModule = await import('https://cdn.jsdelivr.net/npm/@dimforge/rapier2d-compat@0.18.0/rapier.es.js')
    ...
    addWall((left + right) / 2, top - wallThickness, (right - left) / 2, wallThickness)
    addWall((left + right) / 2, bottom + wallThickness, (right - left) / 2, wallThickness)
    addWall(left - wallThickness, (top + bottom) / 2, wallThickness, (bottom - top) / 2)
    addWall(right + wallThickness, (top + bottom) / 2, wallThickness, (bottom - top) / 2)
    ...
    for (let i = 0; i < 42; i += 1) {
      ...
      physicsWorld.createCollider(
        RAPIER.ColliderDesc.cuboid(size / PX_PER_M, size / PX_PER_M).setRestitution(0.62).setFriction(0.55),
        body,
      )
...
// ---------------------------------------------------------------------------
// C: COMPOSITED LIGHT LAYER. Deliberately library-light: prove the rendering
// model first; normal maps/custom shader are the next binding step if C wins.
// ---------------------------------------------------------------------------
const darkness = new Graphics().rect(0, 0, WIDTH, HEIGHT).fill({ color: 0x050309, alpha: MODE === 'C' ? 0.23 : 0 })
lighting.addChild(darkness)
const playerLight = MODE === 'C' ? radialGlow(player.x, player.y, 205, 0x8c5cff, 1.05) : null
const redEmergency = MODE === 'C' ? radialGlow(1110, 180, 260, 0xff214c, 0.82) : null
const blastLight = MODE === 'C' ? radialGlow(0, 0, 320, 0xffd7ac, 1.4) : null
if (MODE === 'C') {
  playerLight.position.set(0, 0)
  blastLight.alpha = 0
  lighting.addChild(playerLight, redEmergency, blastLight)
}
...
const invariantText = new Text({
  text: '120 ZOMBIES · SAME ART · GRAPHICS LAB ONLY',
  style: { fontFamily: 'monospace', fontSize: 10, fill: 0x9b89ad },
})
invariantText.position.set(32, 61)
ui.addChild(invariantText)

ui.addChild(panel(985, 16, 277, 108, 0x07050b, 0.9, 0x583777))
const metricsText = new Text({
  text: 'booting…',
  style: { fontFamily: 'monospace', fontSize: 11, fill: 0xd8caea, lineHeight: 17 },
})
metricsText.position.set(1000, 29)
ui.addChild(metricsText)

if (MODE !== 'A') {
  const scan = new Graphics()
  for (let y = 0; y < HEIGHT; y += 4) scan.rect(0, y, WIDTH, 1).fill({ color: 0xffffff, alpha: 0.018 })
  ui.addChild(scan)
}
...
function triggerBlast(x, y) {
  ...
  blastX = x
  blastY = y
  shake = MODE === 'A' ? 0.16 : MODE === 'B' ? 0.72 : 1.0
  flash = MODE === 'A' ? 0 : MODE === 'B' ? 0.48 : 0.9
  spawnParticleBurst(x, y, MODE === 'B' ? 1050 : MODE === 'C' ? 1450 : 0)
  ...
  if (MODE === 'C' && physicsWorld) {
    ...
    for (const item of physicsDebris) {
      ...
      const strength = Math.min(5.8, 6.8 / d)
      item.body.applyImpulse({ x: (dx / d) * strength, y: (dy / d) * strength }, true)
      item.body.applyTorqueImpulse((rand() - 0.5) * 5.5, true)
    }
  }
}

// Full-screen flash is a cheap but important part of perceived impact.
const flashOverlay = new Graphics().rect(0, 0, WIDTH, HEIGHT).fill({ color: 0xffd7c3, alpha: 1 })
...
let avgFrameMs = 0
let maxFrameMs = 0
let lastAutoblast = performance.now()
...
app.ticker.add((ticker) => {
  ...
  for (const p of particlePool) {
    ...
    p.sprite.alpha = clamp(lifeRatio * 1.4, 0, 1)
  }
  ...
  if (MODE === 'C') {
    const pulse = 0.82 + Math.sin(t * 7.2) * 0.16
    redEmergency.alpha = pulse
    blastLight.position.set(blastX, blastY)
    blastLight.alpha = Math.max(0, flash * 0.82)
  }

  if (shake > 0.002) {
    const mag = shake * (MODE === 'C' ? 15 : 9)
  ...
  flashOverlay.alpha = flash
  flash *= Math.pow(0.015, dt)

  if (AUTOTEST && MODE !== 'A' && now - lastAutoblast > 760) {
    lastAutoblast = now
    const x = 390 + rand() * 510
    const y = 360 + rand() * 170
    triggerBlast(x, y)
  }

  if (now - lastMetricsAt >= 400) {
    avgFrameMs = frameAccum / Math.max(1, frameCount)
    const fps = 1000 / Math.max(0.01, avgFrameMs)
    metricsText.text = [
      `FPS ~ ${fps.toFixed(0)}  AVG ${avgFrameMs.toFixed(1)}ms`,
      `WORST ${maxFrameMs.toFixed(1)}ms`,
...
if (AUTOTEST && MODE === 'A') {
  setInterval(() => { shake = 0.12; eventCount += 1 }, 900)
}
