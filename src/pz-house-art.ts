...
const SAVE_KEY = 'last-block-house-art-v0'
...
type SaveData = {
  ...
  containers: Record<string, InventoryItem[]>
}

const map = houseMapJson as HouseMap

const FRAMES: Record<SpriteKey, [number, number]> = {
...
await app.init({ width: WIDTH, height: HEIGHT, background: '#9ba88d', antialias: true, autoDensity: true, resolution: Math.min(devicePixelRatio || 1, 2) })
...
const textureCache = new Map<SpriteKey, any>()
function textureFor(key: SpriteKey): any {
  const cached = textureCache.get(key)
  if (cached) return cached
  const [cx, cy] = FRAMES[key]
...
function layer(zIndex: number): any {
...
const objectViews = new Map<string, any>()
...
player.zIndex = 999999
const playerShadow = new Sprite(textureFor('rug'))
playerShadow.anchor.set(0.5, 0.5)
playerShadow.scale.set(0.42, 0.16)
playerShadow.tint = 0x20251e
playerShadow.alpha = 0.26
...
const zombies: Zombie[] = [
  { id: 1, x: 2.8, y: 11.8, vx: 0.0007, vy: 0, alive: true },
  { id: 2, x: 15.5, y: 11.3, vx: -0.0007, vy: 0.0004, alive: true },
  { id: 3, x: 2.4, y: 5.7, vx: 0.0004, vy: 0.0006, alive: true },
  { id: 4, x: 15.8, y: 6.2, vx: -0.0005, vy: -0.0003, alive: true },
  { id: 5, x: 11.9, y: 12.8, vx: -0.0006, vy: 0, alive: true },
]
for (const zombie of zombies) {
  const holder = new Container()
  const shadow = new Sprite(textureFor('rug'))
  shadow.anchor.set(0.5)
  shadow.scale.set(0.34, 0.13)
  shadow.tint = 0x263022
  shadow.alpha = 0.25
  const sprite = new Sprite(zombieTexture)
  sprite.anchor.set(0.5, 0.82)
  sprite.scale.set(0.56)
  sprite.tint = 0x9ca695
...
makeDraggable(inventoryWindow)
makeDraggable(lootWindow)
...
function renderWalls(): void {
  for (const spec of map.walls) {
    const view = addWorldSprite(wallLayer, spec.kind === 'exterior' ? 'exterior-wall' : 'interior-wall', spec.x + 0.5, spec.y + 0.5, 'object')
    wallViews.push({ spec, view })
  }
...
function renderObjects(): void {
  for (const object of map.objects) objectViews.set(object.id, addWorldSprite(objectLayer, object.sprite, object.x, object.y, 'object', object.sprite === 'car' ? 0.92 : 1))
...
function renderRoof(): void {
  for (let y = map.roof.y; y < map.roof.y + map.roof.h; y += 1) {
    for (let x = map.roof.x; x < map.roof.x + map.roof.w; x += 1) {
      const roof = addWorldSprite(roofLayer, 'roof', x + 0.5, y + 0.5, 'floor', 1.12)
      roof.tint = (x + y) % 2 === 0 ? 0xffffff : 0xf2e4df
    }
  }
...
function addWorldSprite(target: any, key: SpriteKey, x: number, y: number, kind: 'floor' | 'object', scale = 1): any {
  ...
  sprite.anchor.set(0.5, kind === 'floor' ? 0.43 : 0.78)
...
function isWallCell(x: number, y: number): boolean {
  ...
  return map.walls.some((wall) => wall.x === ix && wall.y === iy)
}
function doorAt(x: number, y: number): OpeningSpec | null {
  const ix = Math.floor(x); const iy = Math.floor(y)
  return map.openings.find((opening) => opening.kind === 'door' && opening.x === ix && opening.y === iy) ?? null
}
function canOccupy(x: number, y: number): boolean {
  ...
  if (door && !doorStates.get(door.id)) return false
  return true
}
...
function nearestDoor(max = 1.15): OpeningSpec | null {
  let best: OpeningSpec | null = null; let bestDistance = max
  for (const opening of map.openings) {
    if (opening.kind !== 'door') continue
    const d = Math.hypot(opening.x + 0.5 - playerX, opening.y + 0.5 - playerY)
    if (d < bestDistance) { best = opening; bestDistance = d }
  }
  return best
}
function nearestContainer(max = 1.25): ObjectSpec | null {
  let best: ObjectSpec | null = null; let bestDistance = max
  for (const object of map.objects) {
    if (!object.container) continue
    const d = Math.hypot(object.x - playerX, object.y - playerY)
...
function interact(): void {
  ...
  if (door) {
    const open = !doorStates.get(door.id)
    doorStates.set(door.id, open)
    const view = openingViews.get(door.id)
    if (view) view.texture = textureFor(open ? 'door-open' : 'door-closed')
    feed(open ? 'Door opened.' : 'Door closed.')
    return
  }
  ...
  if (container) {
    ...
    renderUi()
    feed(`Searching ${container.container}.`)
  }
}

function movePlayer(dx: number, dy: number, sprint: boolean): void {
  const length = Math.hypot(dx, dy)
  if (length < 0.01) return
  const speed = sprint ? 0.052 : 0.034
  const nx = playerX + (dx / length) * speed; const ny = playerY + (dy / length) * speed
  if (canOccupy(nx, playerY)) playerX = nx
  if (canOccupy(playerX, ny)) playerY = ny
}

function updateActors(delta: number): void {
  const pp = iso(playerX, playerY)
  player.position.set(pp.x, pp.y)
  player.zIndex = Math.round(pp.y * 10 + 5)
  for (const zombie of zombies) {
    if (!zombie.alive || !zombie.view) continue
    zombie.x += zombie.vx * delta; zombie.y += zombie.vy * delta
    if (zombie.x < 1 || zombie.x > map.width - 1) zombie.vx *= -1
    if (zombie.y < 1 || zombie.y > map.height - 1) zombie.vy *= -1
    const p = iso(zombie.x, zombie.y)
    zombie.view.position.set(p.x, p.y); zombie.view.zIndex = Math.round(p.y * 10)
  }
}

function updateCutaway(): void {
  const inside = insideHouse()
  roofLayer.alpha += ((inside ? 0.08 : 1) - roofLayer.alpha) * 0.12
  for (const { spec, view } of wallViews) {
    const foreground = spec.kind === 'exterior' && (spec.y >= 8 || spec.x >= 12)
    view.alpha += (((inside && foreground) ? 0.16 : 1) - view.alpha) * 0.16
  }
}
function updateCamera(): void {
  const pp = iso(playerX, playerY)
  world.scale.set(zoom)
  world.position.set(WIDTH * 0.5 - pp.x * zoom, HEIGHT * 0.53 - pp.y * zoom)
}
...
function renderUi(): void {
  weightLabel.textContent = `${inventoryWeight().toFixed(1)} / ${inventoryCapacity()} KG`
  inventoryList.replaceChildren()
  for (const current of inventory) {
    const row = document.createElement('div'); row.className = 'row'
    const copy = document.createElement('div'); copy.innerHTML = `<strong>${escapeHtml(current.name)}</strong><small>${current.weight.toFixed(1)} kg</small>`
    row.appendChild(copy)
    if (current.name === 'School Backpack' && !equippedBackpack) {
      const button = document.createElement('button'); button.textContent = 'EQUIP'
      button.onclick = () => { equippedBackpack = true; feed('Backpack equipped.'); renderUi() }
      row.appendChild(button)
    } else if (['Water Bottle', 'Orange Soda', 'Canned Beans', 'Bandage', 'Painkillers'].includes(current.name)) {
      const button = document.createElement('button')
      button.textContent = current.name === 'Bandage' ? 'USE' : current.name.includes('Water') || current.name.includes('Soda') ? 'DRINK' : current.name === 'Painkillers' ? 'TAKE' : 'EAT'
      button.onclick = () => { inventory = inventory.filter((entry) => entry.id !== current.id); feed(`${current.name} used.`); renderUi() }
      row.appendChild(button)
    }
  ...
  const object = openContainerId ? map.objects.find((candidate) => candidate.id === openContainerId) ?? null : null
  if (!object || !object.container) { lootTitle.textContent = 'CONTAINER'; lootWindow.classList.add('hidden'); return }
  lootTitle.textContent = object.container.toUpperCase()
  const contents = containerItems.get(object.id) ?? []
  for (const current of contents) {
    const row = document.createElement('div'); row.className = 'row'
    const copy = document.createElement('div'); copy.innerHTML = `<strong>${escapeHtml(current.name)}</strong><small>${current.weight.toFixed(1)} kg</small>`
    const button = document.createElement('button'); button.textContent = 'TAKE'; button.onclick = () => takeItem(object.id, current.id)
    row.append(copy, button); lootList.appendChild(row)
  }
...
function takeItem(containerId: string, itemId: number): void {
  const contents = containerItems.get(containerId) ?? []
  const index = contents.findIndex((entry) => entry.id === itemId)
  if (index < 0) return
  const current = contents[index]
  if (inventoryWeight() + current.weight > inventoryCapacity()) { feed('Too heavy.'); return }
  contents.splice(index, 1); inventory.push(current); feed(`Took ${current.name}.`); renderUi()
}
function takeAll(): void {
  if (!openContainerId) return
  const contents = containerItems.get(openContainerId) ?? []
  for (const current of [...contents]) takeItem(openContainerId, current.id)
}

function updatePrompt(): void {
  const door = nearestDoor()
  if (door) { interactionPrompt.textContent = `E · ${doorStates.get(door.id) ? 'CLOSE' : 'OPEN'} DOOR`; return }
  const container = nearestContainer()
  if (container) { interactionPrompt.textContent = `E · SEARCH ${container.container?.toUpperCase()}`; return }
  const room = roomAt(playerX, playerY)
  interactionPrompt.textContent = room ? `${room.id.toUpperCase()} · I INVENTORY · ESC CLOSE PANELS` : 'WASD MOVE · E OPEN / SEARCH · I INVENTORY · WHEEL ZOOM'
}

function save(): void {
  const data: SaveData = { player: { x: playerX, y: playerY }, inventory, equippedBackpack, doorStates: Object.fromEntries(doorStates), containers: Object.fromEntries(containerItems) }
  localStorage.setItem(SAVE_KEY, JSON.stringify(data)); feed('House state saved.')
}
function load(): void {
  const raw = localStorage.getItem(SAVE_KEY)
  if (!raw) { feed('No save found.'); return }
  try {
    const data = JSON.parse(raw) as SaveData
    playerX = data.player.x; playerY = data.player.y; inventory = data.inventory; equippedBackpack = data.equippedBackpack
    for (const [id, value] of Object.entries(data.doorStates)) doorStates.set(id, value)
    for (const [id, value] of Object.entries(data.containers)) containerItems.set(id, value)
    for (const opening of map.openings.filter((entry) => entry.kind === 'door')) { const view = openingViews.get(opening.id); if (view) view.texture = textureFor(doorStates.get(opening.id) ? 'door-open' : 'door-closed') }
    feed('House state loaded.'); renderUi()
  } catch { feed('Save could not be loaded.') }
}

function feed(copy: string): void {
  const line = document.createElement('div'); line.className = 'feed'; line.textContent = copy; eventFeed.prepend(line)
...
function makeDraggable(element: HTMLElement): void {
  const handle = element.querySelector<HTMLElement>('.window-handle')
  if (!handle) return
  let drag = false; let ox = 0; let oy = 0
  handle.addEventListener('pointerdown', (event) => {
    if ((event.target as HTMLElement).tagName === 'BUTTON') return
    drag = true; ox = event.clientX - element.offsetLeft; oy = event.clientY - element.offsetTop; handle.setPointerCapture(event.pointerId)
  })
  handle.addEventListener('pointermove', (event) => {
    if (!drag) return
    element.style.left = `${Math.max(8, Math.min(innerWidth - element.offsetWidth - 8, event.clientX - ox))}px`
    element.style.top = `${Math.max(62, Math.min(innerHeight - 80, event.clientY - oy))}px`; element.style.right = 'auto'
  })
...
window.addEventListener('keydown', (event) => {
  ...
  if (event.code === 'KeyI' || event.code === 'Tab') { event.preventDefault(); inventoryWindow.classList.toggle('hidden'); renderUi() }
  if (event.code === 'Escape') { inventoryWindow.classList.add('hidden'); lootWindow.classList.add('hidden'); openContainerId = null }
})
...
app.ticker.add(() => {
  const now = performance.now(); const delta = Math.min(34, now - last); last = now
  if (inventoryWindow.classList.contains('hidden') && lootWindow.classList.contains('hidden')) {
    const dx = Number(keys.has('KeyD')) - Number(keys.has('KeyA')); const dy = Number(keys.has('KeyS')) - Number(keys.has('KeyW'))
    movePlayer(dx, dy, keys.has('ShiftLeft') || keys.has('ShiftRight'))
  }
  if (openContainerId) {
    const object = map.objects.find((candidate) => candidate.id === openContainerId)
    if (!object || Math.hypot(object.x - playerX, object.y - playerY) > 1.7) { openContainerId = null; lootWindow.classList.add('hidden') }
  }
  updateActors(delta); updateCutaway(); updateCamera(); updatePrompt()
})

renderUi(); updateCamera(); setTimeout(() => bootStatus.classList.add('hidden'), 1800)
Object.assign(window, { __PZ_HOUSE_ART_V0_READY__: true, __PZ_HOUSE_ROOM__: () => roomAt(playerX, playerY)?.id ?? 'outside' })
app.canvas.focus()
