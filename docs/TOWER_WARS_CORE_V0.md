...
- Valid creep sends cost gold and increase future income by a declared amount.
...
### G3 — AUTHORITATIVE_SEND_INCOME_V0
...
At least the 3 frozen tower archetypes can acquire legal targets and apply deterministic damage/cadence semantics. A killed creep cannot subsequently leak.

### G6 — MATCH_TERMINATION_V0
...
Replaying the same ordered action log from the same seed produces the same authoritative state hash and terminal result. Divergence must fail the test.

### G8 — BROWSER_VERTICAL_SLICE_V0

The actual OpenRealm Wasm build boots in Chromium and visibly proves the loop: at least one tower placement changes routing, at least one send spawns a moving creep, at least one creep is killed or leaks, economy/lives change through authoritative state, and the browser remains stable.

`TOWER_WARS_CORE_V0 = PASS` requires G1–G8.
